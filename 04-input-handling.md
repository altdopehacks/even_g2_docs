...
SDKの `OsEventTypeList` に以下のイベントが存在する可能性があります。動作は未確認です。

| 名前 | 説明 |
|---|---|
| `RING_CLOCKWISE_EVENT` | R1リングの時計回り回転 |
| `RING_COUNTER_CLOCKWISE_EVENT` | R1リングの反時計回り回転 |

実機で確認できた場合は以下のパターンで検出できます：

```typescript
bridge.onEvenHubEvent(event => {
  const type = event.sysEvent?.eventType ?? 0
  // 実際のeventType値は実機で console.log して確認
